    ...
    <Card className='w-full'>
      <div
        ...
        className='flex flex-col-reverse justify-between duration-300'>
          {/* lg:p-8 p-4 flex flex-col w-full --ed */}
        <div className='lg:p-1 p-0 flex flex-col w-full'>
          {/* 文章标题 */}
          {/* className={`cursor-pointer text-3xl ${showPreview ? 'text-center' : ''} leading-tight text-gray-700 dark:text-gray-100 hover:text-blue-500 dark:hover:text-blue-400`}> --ed*/}
          <Link
            {...aosProps}
            href={post?.href}
            passHref
            className={`cursor-pointer text-sm ${showPreview ? 'text-center' : ''} leading-tight text-gray-700 dark:text-gray-100 hover:text-blue-500 dark:hover:text-blue-400`}>
...
{/* text-right border-t pt-8 border-dashed */}
          <div className='text-right  pt-0 border-none '>
          {/* hover:bg-opacity-100 hover:underline transform duration-300 p-3 text-white bg-gray-800 cursor-pointer --ed*/}
            {/* <Link
              href={post?.href}
        ...
        {siteConfig('NEXT_POST_LIST_COVER', null, CONFIG) &&
          post?.pageCoverThumbnail && (
            <Link href={post?.href} passHref legacyBehavior>
              ...
              {/* h-72 w-full relative duration-200 cursor-pointer transform overflow-hidden ed */}
              <div className='h-72 w-full relative duration-200 cursor-pointer transform overflow-hidden rounded-2xl'>
                <Image
